...
func foo(e error) {
	fmt.Printf("foo ran the custom error: %v\n. In order to that explicitly says the type of error is customErr and access the field in that specific type, use e.(type).field as in this example: %v\t%v\t%v\n\n", e, e.(customErr).idx, e.(customErr).last, e.(customErr).first)
}
